...
async def admin_products_menu(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        ...
        return
    if callback.message:
        await callback.message.edit_text("Mahsulotlar bo'limi:", reply_markup=admin_products_keyboard())
    await callback.answer()
...
async def admin_customers_menu(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        ...
        return
    if callback.message:
        await callback.message.edit_text("Mijozlar bo'limi:", reply_markup=admin_customers_keyboard())
    await callback.answer()
...
async def admin_requests_menu(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if not requests:
        if callback.message:
            await callback.message.edit_text("Hozircha kutilayotgan buyurtma so'rovlari yo'q.", reply_markup=admin_back_home_keyboard())
        await callback.answer()
        return
...
async def admin_debtors_menu(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if not orders:
        if callback.message:
            await callback.message.edit_text("Hozircha qarzdorlar mavjud emas.", reply_markup=admin_back_home_keyboard())
        await callback.answer()
        return
...
async def admin_history_menu(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if not orders:
        if callback.message:
            await callback.message.edit_text("Hozircha buyurtmalar mavjud emas.", reply_markup=admin_back_home_keyboard())
        await callback.answer()
        return
...
async def admin_reports_menu(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    lines = [f"{month}-{year} hisobot:\n", f"Buyurtmalar soni: {report['order_count']}", f"Jami savdo: {fmt(report['total'])} so'm", f"To'langan: {fmt(report['paid'])} so'm", f"Qarz: {fmt(report['unpaid'])} so'm", "", "Tez sotilayotgan mahsulotlar:"]
    if top_products:
...
async def admin_overdue_menu(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if not orders:
        if callback.message:
            await callback.message.edit_text("Kechikkan qarzlar yo'q.", reply_markup=admin_back_home_keyboard())
        await callback.answer()
        return
    ...
    for order in orders:
        ...
        paid = Decimal(str(order.paid_amount))
        left = total - paid
        days = (now.date() - order.created_at.date()).days
        lines.append(f"Buyurtma ID: {order.id}\nMijoz: {name}\nQoldiq: {fmt(left)} so'm\nHolat: {uzbek_order_status(order.status)}\nKechikish: {days} kun\n")
    if callback.message:
        await callback.message.edit_text("\n".join(lines), reply_markup=admin_back_home_keyboard())
    await callback.answer()
...
async def admin_stock_menu(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if not products:
        if callback.message:
            await callback.message.edit_text("Mahsulotlar yo'q.", reply_markup=admin_back_home_keyboard())
        await callback.answer()
        return
    lines = ["🧮 Qoldiq holati:\n"]
    for p in products:
        lines.append(f"ID: {p.id}\nNomi: {p.name}\nQoldiq: {fmt(p.stock_quantity)} {p.unit}\n")
    if callback.message:
        await callback.message.edit_text("\n".join(lines), reply_markup=admin_back_home_keyboard())
    await callback.answer()