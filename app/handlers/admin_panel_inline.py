...
async def admin_new_sale_hint(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    ...
    if callback.message:
        await callback.message.edit_text(
            "Hozircha admin savdo oqimi eski formatda ishlaydi.\n"
            "Keyingi stepda uni ham to'liq inline qilamiz.",
            reply_markup=admin_back_home_keyboard(),
        )
    await callback.answer()
...
async def admin_payments_hint(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    ...
    if callback.message:
        await callback.message.edit_text(
            "Hozircha to'lov oqimi eski formatda ishlaydi.\n"
            "Keyingi stepda uni ham to'liq inline qilamiz.",
            reply_markup=admin_back_home_keyboard(),
        )
    await callback.answer()
...
async def admin_products_add_hint(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    ...
    if callback.message:
        await callback.message.edit_text(
            "Mahsulot qo'shish hozircha form orqali ishlaydi.\n"
            "Keyingi stepda uni ham inline qilamiz.",
            reply_markup=admin_back_home_keyboard(),
        )
    await callback.answer()
...
async def admin_products_list_inline(callback: CallbackQuery, session: AsyncSession) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
...
async def admin_products_search_hint(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    ...
    if callback.message:
        await callback.message.edit_text(
            "Mahsulot qidirish hozircha eski formatda ishlaydi.\n"
            "Keyingi stepda inline qidiruv qo'shamiz.",
            reply_markup=admin_products_keyboard(),
        )
    await callback.answer()
...
async def admin_products_edit_price_hint(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    ...
    if callback.message:
        await callback.message.edit_text(
            "Narxni o'zgartirish hozircha eski formatda ishlaydi.\n"
            "Keyingi stepda inline tahrirlash qo'shamiz.",
            reply_markup=admin_products_keyboard(),
        )
    await callback.answer()
...
async def admin_products_add_stock_hint(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    ...
    if callback.message:
        await callback.message.edit_text(
            "Qoldiq qo'shish hozircha eski formatda ishlaydi.\n"
            "Keyingi stepda inline qo'shamiz.",
            reply_markup=admin_products_keyboard(),
        )
    await callback.answer()
...
async def admin_products_archive_hint(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    ...
    if callback.message:
        await callback.message.edit_text(
            "Arxivlash hozircha eski formatda ishlaydi.\n"
            "Keyingi stepda inline arxivlash qo'shamiz.",
            reply_markup=admin_products_keyboard(),
        )
    await callback.answer()
...
async def admin_customers_add_hint(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    ...
    if callback.message:
        await callback.message.edit_text(
            "Mijoz qo'shish hozircha eski formatda ishlaydi.\n"
            "Keyingi stepda inline qo'shamiz.",
            reply_markup=admin_customers_keyboard(),
        )
    await callback.answer()
...
async def admin_customers_list_inline(callback: CallbackQuery, session: AsyncSession) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return

    customers = await list_customers(session, limit=50)
    if not customers:
        if callback.message:
            await callback.message.edit_text(
                "Mijozlar yo'q.",
...
async def admin_customers_search_hint(callback: CallbackQuery) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    ...
    if callback.message:
        await callback.message.edit_text(
            "Mijoz qidirish hozircha eski formatda ishlaydi.\n"
            "Keyingi stepda inline qidiruv qo'shamiz.",
            reply_markup=admin_customers_keyboard(),
        )
    await callback.answer()