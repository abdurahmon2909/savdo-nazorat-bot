...
async def approve_request(callback: CallbackQuery, session: AsyncSession):
    ...
    if request.status != "pending":
        ...
        return
    order, error = await approve_order_request(session=session, order_request=request, created_by=callback.from_user.id)
    if error:
        await callback.answer(error, show_alert=True)
        return
    await callback.message.edit_text(f"✅ So'rov tasdiqlandi\n\nSo'rov ID: {request.id}\nBuyurtma ID: {order.id}\nHolat: {uzbek_order_status(order.status)}")
    try:
        await callback.bot.send_message(request.customer_telegram_id, f"✅ Buyurtmangiz tasdiqlandi.\n\nSo'rov ID: {request.id}\nBuyurtma ID: {order.id}\nHolat: {uzbek_order_status(order.status)}")
    except Exception:
        pass
...
async def reject_request_menu(callback: CallbackQuery):
    if not is_admin(callback):
        ...
        return
    request_id = int(callback.data.split(":")[1])
    await callback.message.edit_reply_markup(reply_markup=reject_reason_keyboard(request_id))
...
async def reject_back(callback: CallbackQuery):
    if not is_admin(callback):
        ...
        return
    request_id = int(callback.data.split(":")[1])
    await callback.message.edit_reply_markup(reply_markup=order_request_keyboard(request_id))
    await callback.answer()
...
async def reject_no_reason(callback: CallbackQuery, session: AsyncSession):
    ...
    await callback.message.edit_text(f"❌ So'rov rad etildi\n\nSo'rov ID: {request.id}")
    try:
        await callback.bot.send_message(request.customer_telegram_id, f"❌ Buyurtmangiz rad etildi.\n\nSo'rov ID: {request.id}")
    except Exception:
        pass
...
async def reject_with_reason_start(callback: CallbackQuery, state: FSMContext) -> None:
    if not is_admin(callback):
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    request_id = int(callback.data.split(":")[1])
    ...
    await callback.message.answer(f"So'rov ID {request_id} uchun rad etish izohini yuboring.")
    await callback.answer("Izohni xabar qilib yuboring")


@router.message(ManageOrderRequestState.reject_reason)
async def reject_with_reason_save(message: Message, state: FSMContext, session: AsyncSession) -> None:
