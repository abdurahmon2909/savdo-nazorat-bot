...
async def products_menu(message: Message, state: FSMContext) -> None:
    if not is_admin(message):
        return
    await state.clear()
    await message.answer("Mahsulotlar bo'limi.", reply_markup=products_menu_keyboard())
...
async def cancel_any_product_state(message: Message, state: FSMContext) -> None:
    if not is_admin(message):
        return
    await state.clear()
    await message.answer("Amal bekor qilindi.", reply_markup=products_menu_keyboard())
...
async def products_list(message: Message, session: AsyncSession):
    ...
    await message.answer("\n".join(lines), reply_markup=products_menu_keyboard())
...
async def search_product_start(message: Message, state: FSMContext):
    ...
    await message.answer("Qidiruv uchun nom yuboring", reply_markup=cancel_keyboard())
...
async def edit_product_choose_product(
    ...
    tannarx = f"{format_number(product.cost_price)} so'm" if product.cost_price is not None else "kiritilmagan"
    text_info = (
...
async def edit_product_choose_field(message: Message, state: FSMContext):
    ...
    mapping = {
        "1": "name", "nomi": "name",
        "2": "category", "toifa": "category",
        "3": "unit", "birlik": "unit", "o'lchov": "unit",
        "4": "sell_price", "narx": "sell_price",
        "5": "cost_price", "tannarx": "cost_price",
        "6": "stock_quantity", "qoldiq": "stock_quantity",
    }
    field = mapping.get(field_raw)
    if field is None:
        await message.answer("Maydon noto'g'ri.\nVariantlar: nomi, toifa, birlik, narx, tannarx, qoldiq")
        return
...
async def edit_price_start(message: Message, state: FSMContext, session: AsyncSession):
    ...
    lines = ["Mahsulotlar:\n"]
    for p in products:
        lines.append(f"ID: {p.id}\nNomi: {p.name}\nNarx: {format_number(p.sell_price)} so'm\nQoldiq: {format_number(p.stock_quantity)} {p.unit}\n")
    await state.set_state(EditProductPriceState.product_id)
    await message.answer("\n".join(lines) + "\nNarxini o'zgartirish uchun mahsulot ID raqamini yuboring.", reply_markup=cancel_keyboard())
...
async def edit_price_save(
    ...
    await state.clear()
    await message.answer(f"Narx yangilandi.\n\nMahsulot: {product.name}\nYangi narx: {format_number(product.sell_price)} so'm", reply_markup=products_menu_keyboard())
...
async def add_stock_start(message: Message, state: FSMContext, session: AsyncSession):
    ...
    await message.answer("\n".join(lines) + "\nQoldiq qo'shish uchun mahsulot ID raqamini yuboring.", reply_markup=cancel_keyboard())
...
async def add_stock_save(
    ...
    await state.clear()
    await message.answer(f"Qoldiq qo'shildi.\n\nMahsulot: {product.name}\nYangi qoldiq: {format_number(product.stock_quantity)} {product.unit}", reply_markup=products_menu_keyboard())
...
async def archive_product_start(message: Message, state: FSMContext, session: AsyncSession):
    ...
    await message.answer("\n".join(lines) + "\nArxivlash uchun mahsulot ID raqamini yuboring.", reply_markup=cancel_keyboard())
...
async def archive_product_choose(
    ...
    await message.answer(f"Mahsulotni arxivlaysizmi?\n\nMahsulot: {product.name}\n\nTasdiqlash uchun: ha\nBekor qilish uchun: yo'q")
...
async def archive_product_confirm(
    ...
    await state.clear()
    await message.answer(f"Mahsulot arxivlandi.\n\nMahsulot: {product.name}", reply_markup=products_menu_keyboard())