...
from app.utils.helpers import is_admin  # faqat is_admin kerak
...
async def add_customer_phone(
    ...
    if existing_customer:
        await message.answer("Bu telefon raqam bilan mijoz allaqachon mavjud.\nBoshqa raqam kiriting yoki amalni bekor qiling.")
        return
...
async def customers_list(message: Message, session: AsyncSession) -> None:
    ...
    if not customers:
        await message.answer("Hozircha mijozlar mavjud emas.", reply_markup=customers_menu_keyboard())
        return
    lines = ["So'nggi mijozlar ro'yxati:\n"]
    for index, customer in enumerate(customers, start=1):
        lines.append(f"{index}. {customer.full_name}\n   Telefon: {customer.phone}\n   Holat: {customer.status}\n")
...
async def search_customer_query(
    ...
    if not customers:
        await message.answer("Hech narsa topilmadi.", reply_markup=customers_menu_keyboard())
        await state.clear()
        return
    lines = ["Topilgan mijozlar:\n"]
    for index, customer in enumerate(customers, start=1):
        lines.append(f"{index}. {customer.full_name}\n   Telefon: {customer.phone}\n   Holat: {customer.status}\n   Manzil: {customer.address or 'kiritilmagan'}\n")
    await message.answer("\n".join(lines), reply_markup=customers_menu_keyboard())
    await state.clear()